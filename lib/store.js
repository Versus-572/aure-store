...
const DEFAULT_SETTINGS = {
  footer: {
    brandTagline: 'Premium streetwear, made for the ones who move different.',
    email: 'hello@aure.store',
    country: 'India',
    currency: 'INR',
    copyright: '© 2026 AURE — MADE IN INDIA. ALL PRICES IN INR.',
    shopLinks: [
      { label: 'All', url: '/shop' },
      { label: 'Tees', url: '/shop?category=Tees' },
      { label: 'Hoodies', url: '/shop?category=Hoodies' },
      { label: 'Jackets', url: '/shop?category=Jackets' }
    ],
    helpLinks: [
      { label: 'Cart', url: '/cart' },
  ...
  marquee: 'STREETWEAR • //LIMITED DROPS • AURE • 2026'
};

function getSettings() {
  const saved = readJson(SETTINGS_FILE, {});
  const savedFooter = (saved && saved.footer) ? saved.footer : {};
  return {
    footer: { ...DEFAULT_SETTINGS.footer, ...savedFooter },
    marquee: (saved && saved.marquee) ? saved.marquee : DEFAULT_SETTINGS.marquee
  };
}
...
function updateSettings(data) {
  ...
  const updated = {
    ...
    marquee: data.marquee !== undefined ? data.marquee : current.marquee
  };
